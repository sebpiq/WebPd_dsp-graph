...
export const ensureNode = (graph: PdDspGraph.Graph, nodeId: PdDspGraph.NodeId, jsonNode: PdJson.Node) => {
    if (!graph[nodeId]) {
        graph[nodeId] = {
            id: nodeId,
...
export const connect = (
    ...
    if (!source.sinks.some((otherConnection) => _connectionsEqual(connection, otherConnection))) {
...
export const disconnectNodes = (
    ...
    if (!graph[sourceNodeId] || !graph[sourceNodeId]) {
        debugger
        throw new Error(`both '${sourceNodeId}' and '${sinkNodeId}' must exist in graph`)
    }
...
export const deleteNode = (graph: PdDspGraph.Graph, nodeId: PdDspGraph.NodeId) => {
    const node = graph[nodeId]
    if (!node) {
        return
    }
    // `slice` is required here because we change the list while iterating
    node.sources.slice(0).forEach((connection) => 
...
const _connectionsEqual = (c1: PdDspGraph.Connection, c2: PdDspGraph.Connection) => 
