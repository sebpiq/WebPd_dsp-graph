...
export const testGraphIntegrity = (graph: PdDspGraph.Graph) => {
    const inconsistentConnections: Array<[PdDspGraph.PortletAddress, PdDspGraph.PortletAddress]> = []
    ...
    Object.keys(graph).forEach((nodeId) => {
        ...
        mapNodeSources(graph, nodeId, (sourceAddress, sinkAddress) => {
            const sinks = getSinks(graph, sourceAddress.id, sourceAddress.portlet)
            const matchedSink = sinks.filter(otherSinkAddress => portletAddressesEqual(otherSinkAddress, sinkAddress))[0]
            if (!matchedSink) {
        ...
        mapNodeSinks(graph, nodeId, (sinkAddress, sourceAddress) => {
            const matchedSource = getSource(graph, sinkAddress.id, sinkAddress.portlet)
            if (!matchedSource) {
                inconsistentConnections.push([sourceAddress, sinkAddress])
            }
        })
...
): Array<PdDspGraph.PortletAddress> => getNode(graph, nodeId).sinks[outlet] || []

export const getSource = (
    graph: PdDspGraph.Graph,
    nodeId: PdDspGraph.NodeId,
    inlet: PdDspGraph.PortletId
): PdDspGraph.PortletAddress | undefined => getNode(graph, nodeId).sources[inlet]

export const mapNodeSources = <T>(
    graph: PdDspGraph.Graph,
    nodeId: PdDspGraph.NodeId,
    callback: (sourceAddress: PdDspGraph.PortletAddress, sinkAddress: PdDspGraph.PortletAddress) => T
) => {
...
export const mapNodeSinks = <T>(
    ...
    const node = getNode(graph, nodeId)
    return Object.entries(node.sinks).reduce((previousResults, [outlet, sinkAddresses]) => {
        return [...previousResults, ...sinkAddresses.map((sinkAddress) =>
            callback(sinkAddress, {id: nodeId, portlet: parseInt(outlet, 10)})
        )]
    }, [] as Array<T>)
}